...
import com.Docteur.Enterprise.Repositories.JwtRepository;
import com.Docteur.Enterprise.Repositories.RefreshTokenRepository;
...
public class JwtService {
    ...
    public SignupResponseDto generateAndSaveToken(Employee employee) {
           log.info("Generating new JWT and RefreshToken for user: {}", employee.getEmail());
        // désactiver anciens tokens
        disableToken(employee);

        // générer nouveau JWT
        String jwtBearer = this.generateToken(employee);
        ...
        log.info("JWT and RefreshToken saved for user {}", employee.getEmail());
    ...
    public void disableToken(Employee employee) {
        List<Jwt> jwtList = jwtRepository.findByEmployee(employee.getEmail())
                .peek(jwt -> {
    ...
    //production de refresh Token
     public SignupResponseDto refreshtoken(RefreshTokenDto refreshTokenDto) {
        ...
        SignupResponseDto tokens=  this.generateAndSaveToken(jwt.getEmployee());
...
//deconnexion d'un User ou Admin 
    ...
    try {
        ...
        Optional<Jwt> jwtOpt = jwtRepository.findByEmployeeAndExpirationFalseAndDesactiveFalse(employee);
        ...
        if (jwtOpt.isPresent()) {
            ...
            if (!jwt.getDesactive() && !jwt.getExpiration()) {
                ...
                return ResponseEntity
                        ...
                        .body(new ResponseDto(200, employee.getUsername() + " EST DECONNECTE",""));
            } else {
                return ResponseEntity
                        .status(HttpStatus.BAD_REQUEST)
                        .body(new ResponseDto(401, employee.getUsername() + " EST DEJA DECONNECTE",""));
            }
        } else {
            return ResponseEntity
